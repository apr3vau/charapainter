      ...
      (let* ((point (editor-pane-point board))
        ...
        (labels ((set-new-selected (new)
                   (let ((tool (find 'select tools :key #'class-name-of)))
                     ...
                     (setf (capi:simple-pane-cursor board) #+darwin :closed-hand #-darwin :move)
...
(defun layer-term-color-spec (layer term-color)
  (color:color-to-premultiplied
   (color:color-with-alpha (term-color-spec term-color)
                           (* (term-color-alpha term-color) (layer-alpha layer)))))
...
(defun board-layers-with-selection (board)
  (if (and @board.selected-pixels (pixels-not-empty-p @board.selected-pixels))
    (let* ((layers (project-layers @board.project))
           ...
           (temp-layer (make-layer :pixels (union-pixels @board.current-layer.pixels
                                                         @board.selected-pixels))))
      (append (subseq layers 0 index) (list temp-layer) (subseq layers (1+ index))))
    @board.project.layers))

(defun composed-pixel (x y layers)
  (let (bg fg char bold-p italic-p underline-p)
    (dolist (layer layers)
      (when (layer-visible layer)
        (when-let (pixel (nfind-pixel x y (layer-pixels layer)))
          (awhen (pixel-bg pixel)
            (if bg
              (setq bg (compose-two-colors (layer-term-color-spec layer it) bg))
              (setq bg (layer-term-color-spec layer it))))
          (awhen (pixel-fg pixel)
            (if fg
              (setq fg (compose-two-colors (layer-term-color-spec layer it) fg))
              (setq fg (layer-term-color-spec layer it))))
          ...
          (when (or (null left)
            ...
            (if-let (prev-pixel (nfind-pixel %x %y result))
                (let* ((old-fg (pixel-fg prev-pixel))
                       ...
                       (new-fg-spec (when new-fg (layer-term-color-spec layer new-fg)))
                       (new-bg-spec (when new-bg (layer-term-color-spec layer new-bg)))
              ...
              (let ((pixel (copy-pixel %pixel)))
                (when (pixel-fg %pixel)
                  (setf (pixel-fg pixel)
                        (term-color-from-spec (layer-term-color-spec layer (pixel-fg %pixel)))))
                (when (pixel-bg %pixel)
                  (setf (pixel-bg pixel)
                        (term-color-from-spec (layer-term-color-spec layer (pixel-bg %pixel)))))
                (add-pixel %x %y pixel result)))))))
    (case bit
